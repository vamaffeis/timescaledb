...
typedef struct
{
	...
	Oid watermark_function_oid;	  // Oid of the watermark function
...
constify_cagg_watermark_walker(Node *node, ConstifyWatermarkContext *context)
{
	...
	if (IsA(node, FuncExpr))
	{
		...
		if (context->watermark_function_oid == funcExpr->funcid)
		{
			/* No coalesce expression found so far, we are not interested in this expression */
			if (context->parent_coalesce_expr == NULL)
			{
				context->valid_query = false;
				return false;
			}

			/* Our function takes exactly one argument */
...
replace_watermark_with_const(ConstifyWatermarkContext *context)
{
			 ...
			 context->watermark_functions)
	{
		...
		Assert(context->watermark_function_oid == watermark_function->funcid);
...
constify_cagg_watermark(Query *parse)
{
	...
	context.watermark_function_oid = get_watermark_function_oid();
	context.valid_query = true;

	Ensure(OidIsValid(context.watermark_function_oid),
		   "unable to determine watermark function Oid");
